...
ci_prepare() {
  ...
  if [[ ${STEPS_ACTIVE_MAP["isort"]} == "true" ]] || [[ ${STEPS_ACTIVE_MAP["pytest"]} == "true" ]]; then
...
function run_steps() {
  ...
  if [[ ${STEPS_ACTIVE_MAP[black]} == "true" ]]; then
    echo "Running black..."
    eval "black -l 120 ${BLACK_ACTION} dagger ${STEP_EXTRA_ARGUMENTS[black]} tests integration_tests"
  fi
  ...
  if [[ ${STEPS_ACTIVE_MAP[flake8]} == "true" ]]; then
    echo "Running flake8..."
    eval "flake8 --ignore=W503,E501,F401 ${STEP_EXTRA_ARGUMENTS[flake8]} dagger"
  fi

  if [[ ${STEPS_ACTIVE_MAP[mypy]} == "true" ]]; then
    echo "Running mypy..."
    eval "mypy --no-strict-optional ${STEP_EXTRA_ARGUMENTS[mypy]} dagger"
  fi

  if [[ ${STEPS_ACTIVE_MAP[bandit]} == "true" ]]; then
    echo "Running bandit..."
    eval "bandit --skip=B311 -r ${STEP_EXTRA_ARGUMENTS[bandit]} dagger"
  fi
}
